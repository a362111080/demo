...
public class GoodsController {
    ...
    @ResponseBody
    @LoginToken
    public Message uploadpics(HttpServletRequest request) {
        LoginUser user = (LoginUser) request.getAttribute(ApiConstants.LOGIN_USER);
        Message message = new Message();
        ImageHolder thumbnail = null;
        String thumbnailAddr;
        ...
        try {
            ...
            if (commonsMultipartResolver.isMultipart(request)) {
                ...
                MultipartHttpServletRequest multipartHttpServletRequest = (MultipartHttpServletRequest) request;
                //取出缩略图并构建ImageHolder对象,从MultipartHttpServletRequest中
                List<MultipartFile> thumbnails = multipartHttpServletRequest.getFiles("thumbnail");
                if (thumbnails.size() > 0) {
                    for (MultipartFile thumbnailFile : thumbnails) {
                        thumbnail = new ImageHolder(thumbnailFile.getOriginalFilename(), thumbnailFile.getInputStream());
                        //如果商品缩略图不为null,则添加
                        if (thumbnail != null && null != thumbnail.getImage()) {
