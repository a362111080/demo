...
//        }
        //获取所有分配的角色
        List<Role> roles = roleMapper.selectList(new QueryWrapper<Role>().eq("dr", false));
        for (Role role : roles) {
            List<Resource> resourceList = resourceMapper.selectResourcesByRoleId(role.getId());
            ...
            for (Resource resource : resourceList) {
                //请求url
                String url = !StringUtils.isEmpty(resource.getContent()) ? resource.getContent().trim() : "";
